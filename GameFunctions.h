...
public:
	...
	Vector get_hitbox_location(IClientEntity* obj, int hitbox_id) {
		...
		{
			studiohdr_t* hdr = m_pModelInfo->GetStudioModel(obj->GetModel());
			...
			{
				//Just Hitscan
				math.vector_transform(hitbox->bbmin, matrix[hitbox->bone], vMin);
				math.vector_transform(hitbox->bbmax, matrix[hitbox->bone], vMax);
				...
				if (Shonax::settingsxd.multipoint)
				{
					int iCount = 7;
					for (int i = 0; i <= iCount; i++)
					{
						...
						case 1:
							vTargetPos = (Point[7] + Point[1]) * Shonax::settingsxd.pointscale; break;
						case 2:
							vTargetPos = (Point[3] + Point[4]) * Shonax::settingsxd.pointscale; break;
						case 3:
							vTargetPos = (Point[4] + Point[0]) * Shonax::settingsxd.pointscale; break;
						case 4:
							vTargetPos = (Point[2] + Point[7]) * Shonax::settingsxd.pointscale; break;
						case 5:
							vTargetPos = (Point[6] + Point[7]) * Shonax::settingsxd.pointscale; break;
						case 6:
							vTargetPos = (Point[5] + Point[3]) * Shonax::settingsxd.pointscale; break;
						case 7:
							vTargetPos = (Point[1] + Point[2]) * Shonax::settingsxd.pointscale; break;
						}
						return vTargetPos;
					}
				}
	...
	void trace_line(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, const IClientEntity *ignore, int collisionGroup, trace_t *ptr)
	{
		...
		static UTIL_TraceLine_t TraceLine = (UTIL_TraceLine_t)search.signature("client_panorama.dll", "55 8B EC 83 E4 F0 83 EC 7C 56 52");
	...
	bool trace_to_exit(Vector& end, trace_t& tr, Vector start, Vector vEnd, trace_t* trace)
	{
		...
		static TraceToExitFn TraceToExit = (TraceToExitFn)search.signature(XorStr("client_panorama.dll"), XorStr("55 8B EC 83 EC 30 F3 0F 10 75"));
	...
	bool is_breakable_ent(IClientEntity *m_entity)
	{
		...
		if (!IsBreakableEntity)
			IsBreakableEntity = (isBreakbaleEntityFn)search.pattern(XorStr("client_panorama.dll"), (BYTE*)(XorStr("\x55\x8B\xEC\x51\x56\x8B\xF1\x85\xF6\x74\x68")), XorStr("xxxxxxxxxxx"));
	...
	{
		static DWORD dwAddress = search.signature("client_panorama.dll", "53 8B DC 83 EC 08 83 E4 F0 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 81 EC ? ? ? ? 8B 43 10");
