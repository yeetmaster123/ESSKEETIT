...
static bool BeginChildExNew(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiWindow* window = ImGui::GetCurrentWindow();
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
	...
	if (extra_flags == ImGuiWindowFlags_ChildZeus)
	{
		ImVec2 WinPos = ImGui::GetWindowPos();
		ImVec2 child_bb(window->DC.CursorPos.x, window->DC.CursorPos.y);

		window->DrawList->AddText(child_bb + ImVec2((size_arg.x / 2) - 30, -12), ImColor(255, 255, 255, 255), name);
	}

	return ret;
}

bool ImGui::BeginChildNew(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	return BeginChildExNew(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
...
bool ImGui::MyButtonExXD(const char* label, const ImVec2& size_arg, bool inuse, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	//RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
	if (!inuse)
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 4), ImVec2(bb.Max.x, bb.Max.y), col);
	else
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 4), ImVec2(bb.Max.x, bb.Max.y), col);
...
bool ImGui::MyButton(const char* label, bool inuse, const ImVec2& size_arg)
{
	return MyButtonExXD(label, size_arg, inuse, 0);
}
...
bool ImGui::newSliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color, ImVec2 valuesize, const char* label, char* value)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
...
bool ImGui::newSliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power, ImVec2 size, ImVec4 color)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const bool value_changed = newSliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, 0, color, CalcTextSize(value_buf, NULL, true), value_buf_end, value_buf);
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* display_format)
{
    if (!display_format)
        display_format = "%.0f";
    float v_f = (float)*v;
    bool value_changed = newSliderFloat(label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
    *v = (int)v_f;
    return value_changed;
}
...
void ImGui::RenderCheckMark1(ImVec2 pos, ImU32 col, float sz)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
...
{
	switch (Shonax::settingsxd.buttontype)
	{
	case 0:
	{
		ImGuiWindow* window = GetCurrentWindow();
		...
		RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_CheckMarkHover : ImGuiCol_CheckMarkBg), true, style.FrameRounding);
		if (*v)
		{
			const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
			const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
			RenderCheckMark1(check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), check_bb.GetWidth() - pad * 2.0f);
		}
	...
	case 2:
	{
			...
			if (*v)
			{
				...
				//window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y), check_bb.Max, GetColorU32(ImVec4(0.34f, 1.0f, 0.54f, 1.0f)), 0);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 6, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 5, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 4, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 3, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 2, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 1, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 1, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 2, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 3, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
				window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 4, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.71f, 0.18f, 0.29f, 1.00f)), 12);
	...
	case 3:
	{
		...
		{
			window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Max.y), GetColorU32(ImGuiCol_CheckMark), 0);
...
void ABqdhHWGlevCGVBmelbOhLnRMtbNzgs9756494() {     double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ13231508 = -502165299;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ72585871 = -428041935;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ64972993 = -685734006;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ77770795 = -995512294;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ90243943 = 27571363;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ80114212 = -373445196;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ66970249 = -299423894;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ52052169 = -759897596;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ93427307 = -719363628;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ80353505 = -563234631;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ6121288 = -439358589;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ99769189 = -528415711;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ94947016 = -788275477;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ254907 = -6588705;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ81464859 = -404322648;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ99728844 = -954390184;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ9827284 = -840693879;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ69324007 = -987982142;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ53286856 = -682503198;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ1348099 = 44125614;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ75065661 = 92805581;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ502217 = -981959911;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ96833111 = -540461933;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ44353418 = -125953091;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ89277397 = -524068349;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ69551915 = -741847227;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ19014858 = -643490913;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ88692577 = -821987670;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ47832033 = -8539922;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ24197696 = -500847431;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ90764595 = -789633214;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ59881467 = 14762920;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ53932773 = -356294081;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ43930830 = -949067968;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ66762694 = -417818618;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ56975552 = -840023111;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ48439993 = -687782903;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ82955382 = -302739438;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ36916548 = -893167729;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ62126738 = -82525871;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ17124132 = -659223714;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ80080772 = -724269964;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ38740328 = -794475875;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ63951650 = -235125393;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ42981527 = -966273976;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ91094117 = -578133569;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ3012374 = -905699929;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ62061849 = -954329471;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ24897280 = -317207635;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ44704430 = 65691400;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ83831428 = -731995059;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ96096195 = -177550860;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ12987609 = -552878939;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ9736364 = -126135467;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ34690832 = 71353192;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ12729291 = -520205389;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ75752759 = -887580002;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ20619576 = -459780915;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ88493398 = -371443945;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ20692028 = -230581411;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ61099355 = -729954283;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ78277672 = -477436224;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ4220136 = -651357675;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ69229612 = -118516197;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ89588909 = -773601417;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ46239820 = -354121510;    double mcKnUaYtBKJesoOlWOoNkTixoOZRngIGEmJZ